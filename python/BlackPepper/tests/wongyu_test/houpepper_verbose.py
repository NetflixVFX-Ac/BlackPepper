...
class HouPepper:
    ...
    def set_mantra_for_render(self, hip_path, output_path):
        ...
        if root is not None:
            ...
            n.parm('vm_verbose').set(1) #Set verbosity level to 1
            n.parm("execute").pressButton()
            # while n.isRendering():
            #     print("Rendering frame:", hou.frame())
            #     hou.updateProgressAndCheckForInterrupt()
        render_node = hou.node('/out').currentNode()
        while render_node.isRendering():
            if hou.updateBackground():
                render_node.setRenderInterruptFlag(True)
                break
            print("Rendering frame", hou.frame())
            hou.updateProgressAndCheckForInterrupt()
