...
class HouPepper:
    ...
    def check_abc(self):
        file_name = self.abc_path
        if 'abc' not in file_name:
    ...
    def set_cam_create(self):
        # cam_path : ['/cam1/cam1Camera']
        for cam in self.cam_path:
            # cam_node = cam1Camera
            cam_node = hou.node('/obj').createNode('cam', self.name[self.cam_path.index(cam)])
            ...
            if check_resolution:
                ...
                cam_node.parm('resy').set(int(1920 / self.filmaspectratio[0]))
...
for shot in casted_shots:
    hou.hipFile.load(simulation_path)
    ...
    hou_pepper = HouPepper(layout_output_path)
